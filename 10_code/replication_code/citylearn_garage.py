...
def building_loader(data_path, building_attributes, weather_file, \
    solar_profile, building_ids, buildings_states_actions, save_memory = True):
...
class Citylearn(gym.Env):
    def __init__(self, data_path, building_attributes, weather_file, solar_profile, \
        building_ids, buildings_states_actions = None, simulation_period = (0,8759), \
            cost_function = ['ramping','1-load_factor','average_daily_peak','peak_demand','net_electricity_consumption'], \
                central_agent = False, verbose = 0, reward = None):
        ...
        self.buildings, self.observation_spaces, self.action_spaces, self.observation_space, \
            self.action_space = building_loader(data_path, building_attributes, \
                weather_file, solar_profile, building_ids, self.buildings_states_actions)
        ...
        string = str(self.building_ids[0])
        self.obs_space = akro.Dict({string: akro.Box(low=self.observation_spaces[0].low.min(),\
                 high=self.observation_spaces[0].high.max(), \
                     shape =self.observation_spaces[0].shape)})
        for index in range(1,len(self.building_ids)):
            string = str(self.building_ids[index])
            new_space = akro.Dict({string: akro.Box(\
                low=self.observation_spaces[index].low.min(),\
                 high=self.observation_spaces[index].high.max(), \
                     shape =self.observation_spaces[index].shape)})
            self.obs_space = self.obs_space.concat(new_space)
        # create action space
        string = "building" + str(self.building_ids[0])
        self.act_space = akro.Dict({string: akro.Box(low=self.action_spaces[0].low.min(),\
                 high=self.action_spaces[0].high.max(), \
                     shape =self.action_spaces[0].shape)})
        for index in range(1,len(self.building_ids)):
            string = "building" + str(self.building_ids[index])
            new_space = akro.Dict({string: akro.Box(\
                low=self.action_spaces[index].low.min(),\
                 high=self.action_spaces[index].high.max(), \
                     shape =self.action_spaces[index].shape)})
            self.act_space = self.act_space.concat(new_space)
    ...
    def observation_space(self):
        ...
        # for observation in self.observation_spaces:
        #     observation_low.append(observation.low.min())
        #     observation_high.append(observation.high.max())
        #     observation_shape.append(observation.shape)
        # return akro.Box(low=observation_low, high=observation_high, shape=observation_shape)
    ...
    def action_space(self):
        ...
        # action_high = []
        # action_shape = []
        # for action in self.action_spaces:
        #     action_low.append(action.low.min())
        #     action_high.append(action.high.max())
        #     action_shape.append(action.shape)
        # return akro.Box(low=action_low, high=action_high, shape=action_shape)
    ...
    def step(self, actions):
        ...
        if self.central_agent:
            # If the agent is centralized, all the actions for all the buildings are 
            # provided as an ordered list of numbers. The order corresponds to the 
            # order of the buildings as they appear on the file building_attributes.json, 
            # and only considering the buildings selected for the simulation by the user 
            # (building_ids).
            for uid, building in self.buildings.items():
        ...
        if self.central_agent:
            ...
            rewards = reward_function_sa(self.buildings_net_electricity_demand,self.reward)
            ...
            rewards = self.reward_function.get_rewards(self.buildings_net_electricity_demand,reward= self.reward)

...
class reward_function_ma:
    ...
    def get_rewards(self, electricity_demand,reward=None):  
        if reward == None:
            ...
            if reward == 1:
                return (self.reward_fucntion_power1(electricity_demand))
            elif reward == 2:
                return (self.reward_fucntion_power2(electricity_demand))
            else:
                return (self.reward_fucntion_power3(electricity_demand))
        # Single-agent reward
        # reward_ = np.array(electricity_demand)**3.0
        # reward_[reward_>0] = 0
        # return list(reward_)
    def reward_fucntion_power1(self,electricity_demand):
        reward_ = np.array(electricity_demand)
        reward_[reward_>0] = 0
        return list(reward_)
    ...
    # reward function for the single-agent with power of 2
    def reward_fucntion_power2(self,electricity_demand):
        reward_ = np.array(electricity_demand)
        reward_[reward_>0] = 0
        reward_ = -(reward_ ** 2.0)
        return list(reward_)
    ...
    # reward function for the single-agent with power of 2
    def reward_fucntion_power3(self,electricity_demand):
        reward_ = np.array(electricity_demand)**3.0
        reward_[reward_>0] = 0
        return list(reward_)
        # reward_ = -np.array(electricity_demand).sum()
...
# reward function for the single-agent with power of 1
def reward_fucntion_power1(electricity_demand):
    reward_ = np.array(electricity_demand).sum()
...
# reward function for the single-agent with power of 2
def reward_fucntion_power2(electricity_demand):
    reward_ = np.array(electricity_demand).sum()
    reward_ = min(0, reward_)**2
    # reward_ = -np.array(electricity_demand).sum()
    # reward_ = max(0, reward_)
    # reward_ = reward_**3.0
...
# reward function for the single-agent with power of 2
def reward_fucntion_power3(electricity_demand):
    reward_ = np.array(electricity_demand).sum()
    reward_ = min(0, reward_)**3
    # reward_ = -np.array(electricity_demand).sum()
    # reward_ = max(0, reward_)
    # reward_ = reward_**3.0
...
def reward_function_sa(electricity_demand,reward):
    if reward == None:
        reward_ = reward_fucntion_power1(electricity_demand)
    elif reward == 2:
        reward_ = reward_fucntion_power2(electricity_demand)
    else:
        reward_ = reward_fucntion_power3(electricity_demand)

...
class CityLearn(gym.Env):  
    def __init__(self, data_path, building_attributes, weather_file, solar_profile, \
        ...
        self.buildings, self.observation_spaces, self.action_spaces, self.observation_space, self.action_space = \
            building_loader(self.data_path, self.building_attributes, self.weather_file, self.solar_profile, self.building_ids, self.buildings_states_actions)
    ...
    def sample_tasks(self, num_tasks):
        # sample task(we may have more task)
        goals = self.np_random.randint(5, size=num_tasks)
        tasks = [{'zone': goal} for goal in goals]
        return tasks
    
    def set_task(self, task):
        # reset task by changing the name of the data path(chaning climate_zone)
        self.data_path = Path("data/Climate_Zone_"+str(task['zone']))
        self.building_attributes = self.data_path / 'building_attributes.json'
        self.weather_file = self.data_path / 'weather_data.csv'
        self.solar_profile = self.data_path / 'solar_generation_1kW.csv'
        self._task = task
        self.buildings, self.observation_spaces, self.action_spaces, self.observation_space, self.action_space = \
            building_loader(self.data_path, self.building_attributes, self.weather_file, self.solar_profile, self.building_ids, self.buildings_states_actions)
        # self._task = task
    ...
    def reset(self):
        # change based on the possible different task
        self.buildings, self.observation_spaces, self.action_spaces, self.observation_space, self.action_space = \
            building_loader(self.data_path, self.building_attributes, self.weather_file, self.solar_profile, self.building_ids, self.buildings_states_actions)
